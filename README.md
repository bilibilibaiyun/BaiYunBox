...
一个**轻量、离线优先**的 Windows 桌面 TVBox 播放器 —— 点播 + 直播 + 本地视频 + 播客，一个安装包装好即用。

BaiYun Box 兼容 TVBox 单线路源、FongMi 多线路源（CMS JSON）与 M3U / TXT 直播源，内置 libmpv 播放内核（一个内核同时搞定点播/直播流、本地视频与播客音频）。无需部署服务、无需配置 Go / Node 环境。
...
4. **播客**：播客页粘贴 RSS / Apple 地址导入订阅，或搜索节目订阅；剧集可在线播放/下载。
5. **首页**：查看观看历史，点击断点续播。

## 🔧 开发者构建

```powershell
# 1. 下载引擎（libmpv），需 Python 3.9+
...
随包分发的第三方组件（libmpv / FFmpeg）许可见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。
